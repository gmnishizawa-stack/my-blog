...
## はじめに：「CDNなのになぜDNS変更？」という疑問について解説
...
Cloudflareの導入を検討している方から、こんな声をよく聞きます。

確かに、CDNを使うのに、なぜDNSを変更する必要があるのか、最初は分かりにくいですよね。

今回は、この「CDNとDNSの関係」を解説します。  

## 結論：CloudflareはCDNであり、DNSでもある
...
もう少し詳しく説明しましょう。  
あなたがWebサイトにアクセスする時、こんな流れになっています：
...
つまり、**DNS変更をしないと、訪問者はCloudflareに辿り着けない**のです。
...
訪問者「example.comに行きたい」
  ↓
DNS「あなたのサーバー（192.0.2.1）に行ってください」
  ↓
訪問者はあなたのサーバーへ直接アクセス
  ↓
結果：CDN機能もWAFも働かない
...
### つまり：DNS変更 = トラフィックの入口をCloudflareに変える
...
ただし、パーシャルセットアップを選ぶ場合も、**最終的にはフルセットアップへの移行を検討することをおすすめします**。

### Cloudflareの導入を検討されている方へ

**まずはフルセットアップを検討してみてください。**

- 設定は思ったより簡単です
- Cloudflareの自動インポート機能で、既存のDNSレコードがそのまま移行できます
- 無料プランでも、DNSとCDNの両方が使えます

もし組織的な制約やテスト導入の必要がある場合は、パーシャルセットアップから始めることもできます。
...
- **最終的なセキュリティ対策やDNS設定の選択は、あなたのサイトの要件や予算を考慮したうえで、ご自身の責任で判断してください**
