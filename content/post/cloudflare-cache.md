...
この記事では、Cloudflareのキャッシュの基本から実践的な使い方まで、非エンジニア目線で解説します。
...
訪問者がその画像を見るたびに、毎回ウェブサーバーからデータをとりに行くような動きになっています。

Cloudflareがあると、画像をCloudflareのサーバーにキャッシュ（保存）しておいて、次の訪問者には「保存してあったもの」を素早く渡せます。  
このような動作をするものをCDNとよびます。
